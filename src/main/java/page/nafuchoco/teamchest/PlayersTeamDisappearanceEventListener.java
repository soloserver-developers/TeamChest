...
public class PlayersTeamDisappearanceEventListener implements Listener {
    ...
    public PlayersTeamDisappearanceEventListener(ChestsTable chestsTable) {
    ...
    public void onPlayersTeamDisappearanceEvent(PlayersTeamDisappearanceEvent event) {
        try {
            chestsTable.deleteTeamChest(event.getPlayersTeam());
        } catch (SQLException e) {
            TeamChest.getInstance().getLogger().log(Level.WARNING, "An error occurred while deleting inventory data.", e);
        }
    }
}
